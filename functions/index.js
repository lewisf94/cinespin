// ============================================================================
//  CineClub Cloud Functions — server-authoritative round / turn / reset rules
// ----------------------------------------------------------------------------
...
exports.setDeadline = onCall(async (request) => {
  ...
  if (!group.currentFilm || group.currentFilm.movieId !== movieId) {
    throw new HttpsError("failed-precondition", "That film isn't the current one.");
  }
  const deadline = Timestamp.fromMillis(deadlineMs);
  const batch = db.batch();
...
exports.finalizeRound = onCall(async (request) => {
  ...
  if (!everyoneDone && !(force && currentSpinnerMemberId(group) === memberId)) {
    throw new HttpsError("failed-precondition", "Not everyone has watched and rated yet.");
  }
  ...
  for (const groupDoc of groupsSnap.docs) {
    ...
    for (const memberDoc of membersSnap.docs) {
      ...
      const res = await getMessaging().sendEachForMulticast({
        tokens,
        data: {
          title: "CineClub — watch-by reminder",
          body,
          url: "./?g=" + groupDoc.id,
